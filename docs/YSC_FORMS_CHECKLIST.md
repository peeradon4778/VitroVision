# ✅ YSC 2027 — เอกสารแบบฟอร์มที่ต้องส่ง (VitroVision)

> Checklist ติดตามเอกสารการสมัคร YSC 2027 สำหรับโครงงาน **VitroVision** (Application of AI-Based Computer Vision for Analyzing and Predicting the Growth of Tissue Cultured Plants)
> อ้างอิง: [YSC Wizard — คลังแบบฟอร์ม](https://ysc-wizard.vercel.app/forms.html) · สร้าง 2026-08 ตาม template ล่าสุด (`*_200726` / `*_2027`)
> **วิธีใช้:** แก้ status เป็น `[x]` เมื่อทำเสร็จ · `[!]` = ต้องดำเนินการ · `[ ]` = ยังไม่ทำ

---

## 📌 ความเกี่ยวข้องแบบฟอร์มกับ VitroVision (สรุปด่วน)

| ประเภท | ใช้/ไม่ใช้ | เหตุผล |
|---|---|---|
| มนุษย์ (Form 4 / consent) | ❌ ไม่ใช้ | งานศึกษา**พืช** ไม่มีผู้เข้าร่วมวิจัยมนุษย์ |
| สัตว์ (Form 5) | ❌ ไม่ใช้ | ไม่มีสัตว์มีกระดูกสันหลัง |
| Qualified Scientist (2B) | ❌ ไม่ใช้ | ไม่มี IRB/SRC กำหนดให้มี QS |
| สถาบันวิจัยกำกับดูแล (2C) | ❓ ต้องเช็ค | ถ้าทำในแล็บ/สถาบันที่มีการกำกับดูแล (RRI) |
| **Risk Assessment (Form 3)** | ✅ **น่าจะใช้** | งานแล็บ + สารเคมี (media/ฮอร์โมน/สารต้าน) — หน้าเว็บแนะนำให้ใช้ทุกโครงงาน |
| **Research Continuation (Form 6)** | ✅ **ต้องใช้** | VitroVision **v2 ต่อยอดจาก v1** + เคยส่งเวทีอื่น (PRIK/TTFR/Com Vision) |

---

## 🟦 รอบข้อเสนอโครงการ (ส่งก่อน)

### กลุ่ม A — ต้องส่งทุกโครงงาน
- [ ] **1. ข้อเสนอโครงงาน** — `YSC-Proposal_Template_200726.docx` ⚠️*(template ใหม่ 20/07/26 — ต้องเช็คกับของเดิม 040825)*
  - ไฟล์: [ดาวน์โหลด template](https://www.nstda.or.th/ysc/wp-content/uploads/2026/08/YSC-Proposal_Template_200726.docx)
  - 📄 ใน repo: `docs/proposal_th_draft.md` / `.docx` · หน้าปกต้องสร้างจากระบบ SIMS
- [ ] **2. Code of Conduct / PDPA** (ทุกคนในทีม) — [ไฟล์](https://drive.google.com/file/d/1xipl-A42kSo4WEWyvNL_Owq1O9Uxhxi-/view?usp=sharing)
- [ ] **3. Checklist for Adult Sponsor** (อาจารย์ที่ปรึกษา) — [ไฟล์](https://www.nstda.or.th/ysc/wp-content/uploads/2026/08/1-Checklist-for-Adult-Sponsor_2027.pdf)
- [ ] **4. 1A — Student Checklist** (นักเรียน) — [ไฟล์](https://www.nstda.or.th/ysc/wp-content/uploads/2026/08/1A-Student-Checklist-Research-Plan-Instructions_2027.pdf)
- [ ] **5. 1B — Approval Form** (นักเรียน+ผู้ปกครอง+SRC, คนละฉบับ) — [ไฟล์](https://www.nstda.or.th/ysc/wp-content/uploads/2026/08/1B-Approval-Form_2027.pdf)

### กลุ่ม B — ส่งเมื่อเกี่ยวข้อง (VitroVision ต้องเช็ค)
- [ ] **6. Form 3 — Risk Assessment** ← **สำคัญ** (งานแล็บ+สารเคมี) — [ไฟล์](https://www.nstda.or.th/ysc/wp-content/uploads/2026/08/3-Risk-Assessment_2027.pdf)
- [ ] **7. Form 6 — Research Continuation** ← **สำคัญ** (ต่อยอด v1 / เวทีอื่น) — [ไฟล์](https://www.nstda.or.th/ysc/wp-content/uploads/2026/08/6-YSC-Research-Continuation_200726.docx)
- [ ] 2B — Qualified Scientist (ไม่น่าใช้) — [ไฟล์](https://www.nstda.or.th/ysc/wp-content/uploads/2026/08/2B-Qualified-Scientist_2027.pdf)
- [ ] 4 — Human Participants (ไม่น่าใช้) — [ไฟล์](https://www.nstda.or.th/ysc/wp-content/uploads/2026/08/4-Human-Participants_2027.pdf)
- [ ] Human Informed Consent (ไม่น่าใช้) — [ไฟล์](https://www.nstda.or.th/ysc/wp-content/uploads/2026/08/4-Sample-Informed-Consent_2027.pdf)
- [ ] 5 — Animal Research (ไม่น่าใช้) — [ไฟล์](https://www.nstda.or.th/ysc/wp-content/uploads/2026/08/5-แบบฟอร์มทำการวิจัยในสัตว์ทดลอง_220726.docx)

---

## 🟩 รอบชิงชนะเลิศระดับภูมิภาค/ประเทศ (เตรียมไว้ — หลังผ่านข้อเสนอ)

- [ ] **รายงานฉบับสมบูรณ์** — `YSC-Report_Template_200726.docx` → [ไฟล์](https://www.nstda.or.th/ysc/wp-content/uploads/2026/08/YSC-Report_Template_200726.docx)
  - 📄 ใน repo: `docs/report_th_v1.md` (ปรับตาม template ใหม่ เช็ค version 200726)
- [ ] **2A — Student Support Disclosure** ⚠️ **Gen-AI disclosure ที่นี่** — [ไฟล์](https://www.nstda.or.th/ysc/wp-content/uploads/2026/08/2A-Student-Support-Disclosure-Form_2027.pdf)
- [ ] 2C — Regulated Research Institution (เช็คว่าทำใน RRI) — [ไฟล์](https://www.nstda.or.th/ysc/wp-content/uploads/2026/08/2C-Regulated-Research-Institution_2027.pdf)
- [ ] 4 — Human Participants (ไม่น่าใช้) — [ไฟล์](https://www.nstda.or.th/ysc/wp-content/uploads/2026/08/4-Human-Participants_2027.pdf)

---

## ⚠️ หมายเหตุสำคัญ

1. **Template อัปเดต** — เอกสารใน repo อ้าง `YSC-Proposal_Template_040825.docx` / `YSC-Report_Template_...?` ต้องเช็คว่าตรงโครงสร้าง template ล่าสุด `*_200726` หรือไม่
2. **Gen-AI disclosure** — proposal เขียนว่า "ใช้ Gen-AI เต็มระบบ" → ต้องเปิดเผยใน 2A อย่างตรงไปตรงมา (สอดคล้องกับ `docs/proposal_th_draft.md`)
3. **Form 6 (Continuation)** — เป็นความเสี่ยงที่มากที่สุด ควรเตรียม early เพราะเจาะลึก v1 vs v2 + เวทีอื่น
