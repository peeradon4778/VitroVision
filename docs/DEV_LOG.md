...
| เวลา | สิ่งที่ทำ | ไฟล์ | ผลการทดสอบ |
|---|---|---|---|
| เย็น | ปรับข้อเสนอให้ชัดเจนว่าโมเดล SAM3 (zero-shot pilot) กับ U-Net (กลั่นมาใช้งานจริง) เป็นคนละโมเดล: คงผล pilot SAM3 (height_proxy ≥ 0.275 → acc 0.755/sens 0.917) ไว้เป็นแกน + แทรกย่อหน้าใหม่อธิบายว่า U-Net ให้ seg ดี (val_dice ≈ 0.98) แต่จัดกลุ่มความพร้อมปานกลางกว่า (≥ 0.20 → acc 0.653/sens 0.717 จาก 98 ภาพ) และชี้ว่าการวัด 2D มีข้อจำกัด หนุนสมมติฐาน 3D · แก้จุดอ้างเลข 4 จุด (บทคัดย่อ/ผลนำร่อง/H2/เกณฑ์ 5.4) · เกณฑ์ 5.4 ระบุ 0.275=SAM3 และ 0.20=U-Net | `docs/proposal_ysc.md`, `docs/build_clone.py`, `docs/VitroVision_Proposal_YSC.docx` | rebuild ผ่าน `build_clone.py` (100 paras); verify ข้อความ 0.20/0.275/0.755/0.917/SAM3/U-Net เข้า docx ครบ ✓; sync Desktop `VitroVision_ข้อเสนอ.md` + `_YSC_clone.docx` ✓
...
| บ่าย | **แก้ `space/app.py` expose `demo` ที่ module level** — HF Gradio Space ต้องหา `demo` ได้ (เดิมอยู่ใต้ if **main**) + try/except กัน import ไม่พัง | `space/app.py` | py_compileผ่าน; smoke-test fallback+model path ผ่าน ✓
...
| เย็น | **Reframe ข้อเสนอตาม brief** — บทคัดย่อใหม่ (งานต่อเนื่อง เริ่มแล้ว: lit review/ชุดข้อมูล 100 ขวด/นำร่อง SAM3 + แผนจบปีนี้ + U-Net) · softening "ผ่านเป้า H₂" → "ผลนำร่องชี้ความเป็นไปได้ ยังต้องยืนยัน" (จุด 3.7) · H₂ "ยืนยันแล้ว" → "เบื้องต้นชี้ทิศทาง" + rebuild docx + คัด MD/DOCX ลง Desktop (`VitroVision_ข้อเสนอ.md/.docx`) | `docs/proposal_th_draft.md`, `.docx` | rebuild docx ผ่าน; ไฟล์ Desktop อัปเดต ✓ |
| เย็น | **จัดโครงสร้างตาม YSC Template** (อ้างตัวอย่างรุ่นพี่ `editedYSC-Proposal_fractalwall.docx`) — สร้าง `proposal_ysc.md/.docx`: รหัสโครงการ + บทคัดย่อ/บทสรุป(+คำสำคัญ) + บทนำ + 2.วัตถุประสงค์ + 3.สมมติฐาน + 4.วัสดุและสถานที่ + 5.ระเบียบวิธี + 6.วิเคราะห์ข้อมูล + 7.แผน ก.ย.–ธ.ค. + 8.ประโยชน์ + 9.Gen-AI Disclosure + 10.บรรณานุกรม · ไม่ใส่ประวัติผู้พัฒนา/ที่ปรึกษา (ตามสั่ง) · ฟอนต์ TH SarabunPSK 16pt หัวข้อ bold bilingual · คัดลง Desktop (`VitroVision_ข้อเสนอ.md/.docx`) | `docs/proposal_ysc.md`, `.docx` | ตรวจ bold headers ครบ 10 หัวข้อ; ฟอนต์/ขนาดถูกต้อง ✓ |
| เย็น | **เขียนข้อเสนอใหม่ทั้งหมดตาม ref** (ลบของเดิม) — เขียน `proposal_ysc.md` จากศูนย์: โครงสร้างตามรุ่นพี่ 10 หัวข้อ · หัวข้อ 6 เขียนจากมาตรฐาน benchmark AI (mIoU/Dice/F1/precision/recall · accuracy/sensitivity/specificity/F1/MCC/kappa · threshold tuning) · **สร้าง flowchart 3 รูป** (overview/pipeline/validation) ฝังใน docx · DeepSeek ตัวเดียวใส่ถูกที่ · ตารางแผนแบบรุ่นพี่ (เดือน+ผู้รับผิดชอบ) · เพิ่ม image support ให้ `make_proposal_docx.py` | `docs/proposal_ysc.md/.docx`, `docs/assets/flow_*.png`, `docs/make_proposal_docx.py` | docx มี 3 รูปฝัง + header template + footer เลขหน้ากลาง ✓ |
