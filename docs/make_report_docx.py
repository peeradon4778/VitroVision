# -*- coding: utf-8 -*-
"""แปลง docs/report_th_v1.md → docx สไตล์รายงาน (สไตล์เดียวกับ proposal)
- ฟอนต์ TH SarabunPSK ทั้งเล่ม · หัวข้อ bold 16pt · เนื้อหา 16pt
- ตาราง style TableGrid · กระดาษ A4 ขอบ 1" · footer หมายเลขหน้า "หน้า N"
- รองรับ: หัวข้อ #/##/###, บล็อกquote >, ตาราง |, 
  รายการ - / *, รายการเลข 1., บรรณานุกรม (hanging indent), checkbox - [ ]

รัน:  python make_report_docx.py [--in report_th_v1.md] [--out report_th_v1.docx]
"""
import argparse
import pathlib
import re
...
def main():
    ...
    while i < len(lines):
        ...
        if m:
            ...
            i += 1
            continue
        # รายการ bullet / checkbox
        m = re.match(r'^[-*]\s+(.*)$', line)
        if m:
            body = m.group(1)
            bullet = '☐ ' if body.startswith('[ ]') else ('☑ ' if re.match(r'^\[x\]', body) else '• ')
            if body.startswith('[ ]'):
                body = body[3:].strip()
            elif re.match(r'^\[x\]', body):
                body = body[4:].strip()
            p = doc.add_paragraph()
            add_rich(p, bullet + body, size=16)
            set_para(p, after=80, ind_left=720)
            i += 1
            continue
        # บรรทัดว่าง
