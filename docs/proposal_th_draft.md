...
งานนี้สร้างชุดข้อมูลภาพถ่ายจริงของพืชเพาะเลี้ยงเนื้อเยื่อผ่านขวดแก้วชุดแรก (100 ขวด พริกจินดา) พร้อมระบบการวัดเชิงปริมาณ 6 กลุ่ม และประเมินระบบเปรียบเทียบกับวิธีพื้นฐาน (baseline) ได้แก่ SAM2, YOLO-seg และการแบ่งส่วนเชิงคลาสสิก ด้วยตัวชี้วัดมาตรฐาน mIoU, Dice, precision, recall และ F1 พร้อมการวิเคราะห์ความไวของพรอมป์ (prompt sensitivity) และความไวของเกณฑ์การตัดสินใจ (threshold sensitivity) ผลนำร่องนำไปประเมินความสมเหตุผลเชิงชีววิทยาของลักษณะเชิงปริมาณ และประมวลผลชุดข้อมูล 100 ขวด (ได้ผลการจัดกลุ่ม 75 พร้อมอนุบาล / 25 ยังไม่พร้อม โดยเกณฑ์อิงความสูง: accuracy 0.755, sensitivity 0.917) [RESULT] และการประเมินเปรียบเทียบกับวิธีพื้นฐานระดับค่าวัด (trait เทียบมือ) ได้ผลแล้ว: SAM3 r=0.638 ดีกว่า classical 0.498 / YOLO-COCO 0.133 [RESULT — ดู 7.6.0] ส่วนการประเมินระดับพิกเซล (mIoU/Dice) และการสร้างค่าอ้างอิงแบบ mask ยังเป็น [PLAN]
...
**ข้อจำกัดจากผลจริง:** (1) การตรวจจับระบบรากยังไม่ได้ผล — ตรวจพบรากเพียง 1/100 ภาพ จึงไม่ได้ใช้ root_ratio เป็นตัวชี้วัดอันดับ 1 ในรอบนี้ (หันไปใช้ความสูงแทนตามมุมผู้เชี่ยวชาญ) [OPEN — ต้องปรับปรุงพรอมป์/วิธี] (2) ยังไม่ได้สอบเทียบหน่วยทางกายภาพ (canopy_h_cm/canopy_w_cm เป็น NaN) [OPEN — CALIBRATION_GUIDE] (3) ผลเป็น pilot (n = 98 ภาพที่ผู้ประเมินระบุชัด) ยังต้องตรวจกับผู้ประเมินหลายคน (inter-rater) ก่อนยืนยันเป็นข้อสรุป [PLAN] การประเมินเปรียบเทียบกับวิธีพื้นฐานระดับค่าวัด (trait เทียบมือ) ได้ผลแล้ว: SAM3 r=0.638 ดีกว่า classical 0.498 / YOLO-COCO 0.133 (ดู 7.6.0) [RESULT บางส่วน — ความสอดคล้องกับมือ] ส่วนระดับพิกเซล (mIoU/Dice) ยังต้อง annotate ground-truth masks [PLAN]

---

## 4. วัตถุประสงค์

โครงงานนี้ต่อยอดจากผลนำร่อง (3.7) ที่พิสูจน์ความเป็นไปได้ของแนวทางแล้ว สู่การพัฒนาระบบติดตามการเจริญตามเวลา (time-series monitoring):

1. **สร้างชุดข้อมูลภาพถ่ายจริง**ของพืชเพาะเลี้ยงเนื้อเยื่อผ่านขวดแก้ว (≥ 100 ขวด พร้อม metadata และการกำกับภาพโดยมนุษย์บางส่วน) ซึ่งเป็นชุดข้อมูลใหม่ที่ยังไม่มีในงานวิจัยก่อนหน้า

2. **พัฒนาระบบติดตามการเจริญตามเวลา (time-series monitoring)** — ถ่ายภาพขวดซ้ำรายรอบ (รายสัปดาห์) → แบ่งส่วนด้วย SAM3 PCS → เก็บ feature (coverage/ใบ/ราก/สี) → วิเคราะห์อัตราการเจริญและแนวโน้ม (growth curve) → แจ้งเตือนเมื่อต้นพร้อมอนุบาล
...
4. **ตรวจสอบความถูกต้องของระบบ**กับค่าอ้างอิงจากผู้ประเมิน (ground truth) และปรับปรุงการตรวจจับขวด (ROI) ที่เป็นข้อจำกัดของผลนำร่อง
