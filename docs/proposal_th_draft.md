...
**ผลนำร่อง (pilot):** ชุดภาพจริง 100 ขวดพริกจินดาได้เตรียมไว้แล้ว อยู่ระหว่างรันประมวลผลบน Colab — ใช้เพียงสมาร์ตโฟน, Google Colab ฟรี และโมเดล open-source (ต้นทุนต่ำสุด) [PLAN]
...
### 3.7 ผลการทดลองนำร่อง (Pilot Results) [PLAN]

ชุดภาพจริง 100 ขวดพริกจินดา (ชุด `data/raw/20260814_batch`, วันถ่าย 16 ก.ค./2 ส.ค./14 ส.ค. 2569) ได้เตรียมไว้แล้ว อยู่ระหว่างรันประมวลผลบน Colab (SAM3 ต้องใช้ GPU) — ยังไม่ได้ผลลัพธ์จริง [PLAN]

**ผลที่คาดว่าจะได้ (ตามการออกแบบ):**

| กลุ่ม | จำนวนขวด |
|---|---|
| พร้อมอนุบาล | รอผล |
| ยังไม่พร้อม | รอผล |
| ตรวจเอง (ROI ไม่ชัด) | รอผล |

**ความสัมพันธ์เชิงปริมาณ (Pearson r):** รอผลรันจริง

เมื่อได้ไฟล์ `plant_growth_summary.csv` ของชุด 100 แล้วจะอัปเดตตัวเลขจริงในส่วนนี้ [PLAN]

> หมายเหตุ: ต้องรอผลรันจริงบน Colab ก่อนนำตัวเลขอ้างอิง — อยู่ในระหว่างดำเนินการ
...
*ข้อเสนอโครงงานนี้ (ส่วนที่ 1) จัดทำเพื่อส่งประกวด YSC 2027 สาขา CSAI (AI/ML) — เนื้อหาเป็นของผู้พัฒนาโครงงาน โดยผลนำร่องระบุเป็น [PLAN] จนกว่าจะมีผลรันจริงจาก Colab*
