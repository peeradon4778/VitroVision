...
**ภาษาไทย:** VitroVision : การประยุกต์ใช้ปัญญาประดิษฐ์เชิงคอมพิวเตอร์วิทัศน์เพื่อวิเคราะห์และทำนายการเจริญเติบโตของพืชเพาะเลี้ยงเนื้อเยื่อ

**ภาษาอังกฤษ:** VitroVision : Application of AI-Based Computer Vision for Analyzing and Predicting the Growth of Tissue Cultured Plants
...
โครงงานนี้เสนอระบบคัดกรองความพร้อมอนุบาลของต้นกล้าเพาะเลี้ยงเนื้อเยื่อผ่านขวดแก้วโดยใช้การแบ่งส่วนภาพ (segmentation) แบบ zero-shot จากโมเดลพื้นฐาน Segment Anything Model 3 (SAM3) — ปัญหาคอมพิวเตอร์วิทัศน์ที่ท้าทายจากแสงสะท้อน (glare) ไอน้ำ และความโค้งของแก้ว — ร่วมกับการออกแบบพรอมป์ข้อความ 5 คำ (plant, leaf, shoot, stem, root) การตรวจจับขอบเขตขวด (bottle ROI) และขั้นตอนวิธีตัดสินใจแบบกฎที่จัดกลุ่มขวดเป็น ยังไม่พร้อม / พร้อมอนุบาล (โดยภาพที่ประมวลผลไม่ชัดจะส่งให้มนุษย์ตรวจเพื่อความปลอดภัย)

**ผลนำร่อง (pilot):** ชุดภาพจริง 100 ขวดพริกจินดาประมวลผลบน Colab ได้ผลแล้ว (จัดกลุ่ม 75 พร้อมอนุบาล / 25 ยังไม่พร้อม โดยเกณฑ์อิงความสูง: accuracy 0.755, sensitivity 0.917) — ใช้เพียงสมาร์ตโฟน, Google Colab ฟรี และโมเดล open-source (ต้นทุนต่ำสุด) [RESULT]

โครงงานนี้ต่อยอดจากผลนำร่องสู่ **ระบบติดตามการเจริญของต้นกล้าในขวดตามเวลา (time-series monitoring)** สำหรับห้องปฏิบัติการเพาะเลี้ยงเนื้อเยื่อและเรือนเพาะชำ (greenhouse/plant factory) ได้แก่ การวัดซ้ำรายรอบ การวิเคราะห์อัตราการเจริญและแนวโน้ม (growth curve) การแจ้งเตือนเมื่อต้นพร้อมอนุบาล และการตรวจสอบความถูกต้องกับค่าอ้างอิงจากผู้ประเมิน (ground truth) พร้อมการประเมินเปรียบเทียบกับวิธีพื้นฐาน (SAM2, YOLO-seg, การแบ่งส่วนเชิงคลาสสิก) ด้วยตัวชี้วัดมาตรฐาน mIoU, Dice และ F1

งานนี้สร้างชุดข้อมูลภาพถ่ายจริงผ่านขวดแก้วชุดแรก (100 ขวด พริกจินดา) พร้อมระบบการวัดเชิงปริมาณ 6 กลุ่ม และประเมินผลเปรียบเทียบกับวิธีพื้นฐาน (baseline) ได้แก่ SAM2, YOLO-seg และการแบ่งส่วนเชิงคลาสสิก พร้อมการวิเคราะห์ความไวของพรอมป์ (prompt sensitivity) และความไวของเกณฑ์การตัดสินใจ (threshold sensitivity) — ผลเบื้องต้นระดับค่าวัด (trait เทียบมือ) พบว่า SAM3 สอดคล้องกับค่าอ้างอิงจากมือมากที่สุด (ความสูง r = 0.638 ดีกว่า classical 0.498 / YOLO-COCO 0.133) [RESULT — ดู 7.6.0] ส่วนการประเมินระดับพิกเซล (mIoU/Dice) และการสร้างค่าอ้างอิงแบบ mask ยังเป็น [PLAN]
...
**การตรวจสอบกับค่าอ้างอิงจากผู้ประเมิน (validation):** เกณฑ์ความพร้อมอิงความสูง (`height_proxy ≥ 0.275`) ตามนิยาม "ต้นสมบูรณ์/โตพอ/พร้อมย้าย" ของผู้ประเมิน → Accuracy 0.755, **Sensitivity 0.917**, F1 0.821, MCC 0.472 [RESULT — ผ่านเป้า H₂] ส่วนเกณฑ์เดิมที่อิง coverage_ratio ให้ accuracy = 0.43 / sensitivity = 0.15 (ไม่สอดคล้อง)
...
| บัญชี Google Colab (โควตา GPU) | แพลตฟอร์มรันโมเดลฟรี | รัน SAM3 PCS แบบ headless batch บน GPU |
| สิทธิ์เข้าถึง `facebook/sam3` (gated) | Hugging Face account + ยืนยันสิทธิ์เข้าถึงโมเดล | โหลดโมเดล SAM3 PCS มาประมวลผลในเครื่อง (Colab) |
| SAM3 PCS (`facebook/sam3`, Carion et al., 2025) | Promptable Concept Segmentation — รันผ่าน `transformers` | Segment ต้นพืชและใบจากภาพผ่านขวดแก้ว |
| (ทางเลือก) Roboflow SAM3 PCS API | บัญชี Roboflow พร้อม API key — ใช้ใน spike test ระยะแรก | ส่งภาพและรับ mask ผ่าน API เป็นทางเลือก |
...
**7.6.0 ผลที่ได้เบื้องต้น — ระดับค่าวัด (trait/measurement) [RESULT]: `src/benchmark_traits.py`** — เปรียบเทียบ proxy ของ "ขนาดต้น" ที่แต่ละวิธีได้จาก segmentation กับค่าที่วัดมือ (`height_cm`, `area_cm2`) ด้วย Pearson r (scale-free — proxy เป็น px แต่เทียบกับ cm ได้โดยไม่ต้องสอบเทียบหน่วย):
...
|---|---|---|---|---|---|---|---|---|---|---|---|---|
| 9.1 เก็บข้อมูลภาพถ่ายขวดซ้ำรายรอบ (time-series) + metadata | █ | █ | | | | | | | | | | |
| 9.2 สร้าง ground-truth masks (annotate ≥30 ภาพ) + วัดมือ | █ | █ | | | | | | | | | | |
| 9.3 ขยายชุดข้อมูลหลายชนิด (≥2–3 ชนิด, ข้ามชนิด) | | █ | █ | | | | | | | | | |
| 9.4 รัน baseline segmentation (SAM2/YOLO-seg/classical) + mIoU/Dice | | | █ | █ | | | | | | | | |
| 9.5 วิเคราะห์ growth curve (time-series) + ทำนายวันพร้อม | | | | █ | █ | █ | | | | | | |
| 9.6 ปรับ rule-based decision + threshold sensitivity (MCC สูงสุด) | | | | | | █ | █ | | | | | |
| 9.7 prompt sensitivity + validate ข้ามชนิด (inter-rater) | | | | | | | █ | █ | | | | |
| 9.8 เขียนรายงาน + diagram + เปิดชุดข้อมูล (FAIR) | | | | | | | | | █ | █ | | |
| 9.9 ทบทวน + เตรียมส่งรอบภูมิภาค | | | | | | | | | | █ | █ | |
...
### 11.1 เส้นทางต่อยอด (Future Work) [PLAN]
...
## 14. ประวัติย่อผู้พัฒนาโครงงานและอาจารย์ที่ปรึกษา

> **กรุณากรอกข้อมูลจริง** (เป็นส่วนที่ FAQ กำหนดให้มีใน PDF ข้อเสนอโครงงาน — ใช้ข้อมูลจริงของคุณเอง)

### 14.1 ผู้พัฒนาโครงงาน (นักเรียน)

- **ชื่อ-นามสกุล (ภาษาไทย):** \_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_
- **ชื่อ-นามสกุล (ภาษาอังกฤษ):** \_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_
- **ระดับชั้น:** \_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_
- **โรงเรียน:** \_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_
- **อีเมล:** \_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_
- **ประวัติ/ผลงานโดยย่อ:** \_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_

### 14.2 อาจารย์ที่ปรึกษา

- **ชื่อ-นามสกุล (ภาษาไทย):** \_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_
- **ชื่อ-นามสกุล (ภาษาอังกฤษ):** \_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_
- **ตำแหน่ง / สถานที่ทำงาน:** \_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_
- **ความเชี่ยวชาญ:** \_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_
- **อีเมล:** \_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_
- **ประวัติโดยย่อ:** \_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_

---

> **หมายเหตุการจัดส่ง:** หน้าปกข้อเสนอโครงงานต้องสร้างจากระบบ SIMS เท่านั้น แล้วนำมาใส่เป็นหน้าแรกของไฟล์ PDF (ดู `docs/SIMS_SUBMISSION_GUIDE.md`)
